...
const Experience = (props) => {
  const [experience] = useState({
    ...
    Trash2Cash: [
      {
        id: 7,
        position: "Automated Trading System Developer (Freelance)",
        from: "May 2023",
        to: "July 2023",
        tasks: [
    ...
    "Noble Systems": [
      {
        ...
        taskId: 3,
        subTasks: [
          "Troubleshooting of Dialogic, Aculab, and Asterisk telephony systems (VOIP & T1s)",
                      ...
                      <li key={job.id} className="relative p-3">
                        ...
                        <ul className="mt-1 text-xs font-normal list-disc">
                          {job.tasks.map((task, index) => (
                            <li
                              key={`${job.id}-task-${index}`}
                              className="mx-4 my-2"
                            >
                              {task}
                              {job.subTasks && index === job.taskId ? (
                                <ul className="mt-1 text-xs font-normal list-[circle]">
                                  {job.subTasks.map(
                                    (subResponsibility, index) => (
                                      <li
                                        key={`${job.id}-subtask-${index}`}
                                        className="mx-4 my-2"
                                      >
                                        {subResponsibility}
                                      </li>
